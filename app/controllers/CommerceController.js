...
module.exports = {
  index_all_commerces: async (request, response) => {
    try {
      ...
      return response
        ...
        .json({ message: "Could not fetch data", error });
    }
  },
  ...
  create_a_commerce: async (request, response) => {
    try {
      ...
      return response
        .status(status.INTERNAL_SERVER_ERROR)
        .json({ message: "Could not create commerce", error });
    }
  },
  update_a_commerce: async (request, response) => {
    try {
      ...
      if (!deprecated_commerce) {
        throw new Error({
          name: "Commerce not found",
          message: "Could not find the commerce with the requested ID.",
        });
      }
      ...
      const updated_commerce = {
        ...
        city: request.body.city.toLowerCase() || deprecated_commerce.city,
        number: request.body.number || deprecated_commerce.number,
        neighborhood:
          request.body.neighborhood.toLowerCase() ||
          deprecated_commerce.neighborhood,
      ...
      if (updated)
        return response
          .status(status.OK)
          .json({ message: "commerce updated", updated_commerce });
    } catch (error) {
      return response
        .status(status.INTERNAL_SERVER_ERROR)
